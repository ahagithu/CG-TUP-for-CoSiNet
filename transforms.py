...
from siamfc import ops  # from . import ops
...
#主要是随机的resize图片的大小，其中要注意cv2.resize（）的一点用法
...
#用法类似centercrop，只不过从随机的位置区，没有pad的考虑
...
class SiamFCTransforms(object):
    ...
    def _crop(self, img, box, out_size):
        #因为GOT-10k里面对于目标的bbox是以ltwh(即left, top, weight, height)形式给出的，
        # 上述代码一开始就先把输入的box变成center based，坐标形式变为[y, x, h, w]
